...
export class CropperDemoComponent implements OnChanges, OnDestroy {
  ...
  ngOnChanges() {
    ...
    this.subscriptions.push(
      this.cropper.editable.ready.subscribe((r) => {
        this.subscriptions.push(
          this.cropper.configChange.pipe(debounceTime(300)).subscribe((res) => {
            this.croppImage.emit(
              new File([this.cropper.getBlob()], this.file.name)
            );
          })
        );
      })
